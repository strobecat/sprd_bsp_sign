...
int sprd_add_sb_cert(char *img, char *cert, char *key_path, char *id0, char *id1, char *mask, char*pss_flag)
{
	int fd = 0;
	int i;
	...
	{
		printf("error:strdup(img) failed !!!\n");
		return 0;
	}
	...
	if (0 != memcmp("fdl1-sign.bin", name, strlen("fdl1-sign.bin"))
	    ...
	    && 0 != memcmp("u-boot-spl-16k-emmc-sign.bin", name, strlen("u-boot-spl-16k-emmc-sign.bin"))
	    && 0 != memcmp("u-boot-spl-16k-ufs-sign.bin", name, strlen("u-boot-spl-16k-ufs-sign.bin"))) {
...
#else
	sprd_devcert.debug_mask = strtoull(mask, &end, 16);
#endif
	...
	getpubkeyfrmPEM(&sprd_devcert.dev_pubkey, key[0]);
        if(0 == strcmp(pss_flag,"pkcs15")) {
            printf("use pkcs1 format\n");
	    calcSignature_pkcs1(&sprd_devcert.debug_mask, DEVEDBG_HASH_LEN, sprd_devcert.dev_signature, key[1]);
        } else {
            printf("use pss format\n");
            cal_sha256(&sprd_devcert.debug_mask, DEVEDBG_HASH_LEN, certhash);
            calcSignature_pss(certhash, DEVEDBG_HASH_LEN , sprd_devcert.dev_signature, key[1]);
        }
...
fail:
	printf("error: fail\n");
	close(fd);
	if (input_img != NULL) free(input_img);
    if (primary_sb_cert_data != NULL) free(primary_sb_cert_data);
	return 0;
...
int main(int argc, char **argv)
{
	...
	char *cmd1, *cmd2, *cmd3, *cmd4, *cmd5, *cmd6, *cmd7;
	...
	cmd5 = argv[5];
	cmd6 = argv[6];
	cmd7 = argv[7]; //pss or pkcs1_v1.5 flag
	sprd_add_sb_cert(cmd1, cmd2, cmd3, cmd4, NULL, cmd5, cmd7);/*img,primary_cert,key_path,soc_id,mask*/
	return 1;
}
