...
int sprd_create_primary_cert(char *mask, char *key_path, char *pss_flag)
{
	int fd;
	int i;
	...
	#else
	sprd_primary_cert.debug_mask = strtoull(mask, &end, 16);
	sprd_primary_cert.reserved = 0;

	printf("intput mask is: 0x%llx \n", sprd_primary_cert.debug_mask);
	#endif
	...
	cal_sha256(&dev_sb_pubk, SPRD_RSAPUBKLEN, sprd_primary_cert.devkey_debug_hash_data);	/*cal devpubk's hash and keep in primary cert */
	    ...
	    calcSignature_pkcs1(sprd_primary_cert.devkey_debug_hash_data, PRIMDBG_HASH_LEN, sprd_primary_cert.devkey_debug_signature, key[2]);
        } else {
            printf("use pss format\n");
            cal_sha256(sprd_primary_cert.devkey_debug_hash_data, PRIMDBG_HASH_LEN, certhash);
            calcSignature_pss(certhash, PRIMDBG_HASH_LEN, sprd_primary_cert.devkey_debug_signature, key[2]);
        }
...
int main(int argc, char **argv)
{
	...
	char *cmd3 = argv[3];	/*pss or pkcs1_v1.5*/

	sprd_create_primary_cert(cmd1, cmd2, cmd3);
